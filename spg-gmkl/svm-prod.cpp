...
void lookuptable(){
	double delta = 0.0;
	double add = powf(10.0,-LookUp::precision);
...
//  const double inf_abs_tol=1e-3;
 // const double l2_abs_tol=1e-3;
  //const double l2_abs_tol=1e-2;
  const double inf_abs_tol=0.05;
  const double l2_abs_tol=0.05;
...
void copy_param(svm_parameter *new_p,const svm_parameter *param)
{
	...
	new_p->L_p=param->L_p;
	new_p->fperk=param->fperk;
  new_p->l=param->l;
  new_p->features=param->features;
...
int Cache::get_data(const int index, Qfloat **data, int len)
{
	...
	if(more > 0)
	{
		...
		// allocate new space
		Qfloat *dummy;
		dummy = (Qfloat *)realloc(h->data,sizeof(Qfloat)*len);
...
class QMatrix {
public:
	virtual Qfloat *get_Q(int i, int len) const=0;
	virtual Qfloat *get_Q_K(int i, int len, double *alpha1, bool &inner) const=0;
    ...
    virtual int *get_master(void) const=0;
    virtual double kernel_ij(int i, int j) const=0;
};
...
public:
  ...
  double *get_d(void) const { return d; } 
   int *get_master(void) const { return manage_master;}
...
protected:
  ...
  int fperk;
	...
	double *d_repeat;
	int *manage_master; // to manage recently updated dual variables
	Cache **cache;
	Cache *cache1;
	Cache *master_cache;
  double **data_feature;
  ...
  {
	  int start = n*fperk;
	  int end =  min(max_index, start + fperk);
	  double inner = 0.0;
	  for(int k = start; k<end;k++)
			inner+=data_feature[i][k]*data_feature[j][k];
		return (data_square[i][n] + data_square[j][n] - 2.0*inner);	      
  }

	double feature_xk(int pattern, int feature) const{
		return feature_k(pattern,feature);
	}
	
	double kernel_Prodfinal(int i, int j) const {
			return KProdRBF(x[i],x[j]);
	}
...
private:
	...
	double feature_k(int pattern, int n) const {
		const svm_node *px = x[pattern];
		if(px->index == 0) ++px;
		while(px->index != -1 && px->index <= n){
			if (px->index == n)
				return px->value;	
			++px;
		} 
	...
	double KProdRBF(const svm_node *px, const svm_node *py) const
{
	...
	while(px->index != -1 && py->index != -1)
	{
		...
		{	
			if (d_repeat[px->index-1] > DTAU)
				sum+=d_repeat[px->index-1]*(px->value - py->value)*(px->value - py->value);
			++px;
			++py;
		}
		else
		{
			if(px->index > py->index){
				if (d_repeat[py->index-1] > DTAU)
					sum+=d_repeat[py->index-1]*py->value*py->value;
				++py;
			}
			else{
				if (d_repeat[px->index-1] > DTAU)
					sum+=d_repeat[px->index-1]*px->value*px->value;
				++px;
			}
		}			
	}
	if(px->index == -1 && py->index != -1){
		while(py->index != -1){
			sum+=d_repeat[py->index-1]*py->value*py->value;
			++py;
		}			
	}
	else if(px->index != -1 && py->index == -1){
		while(px->index != -1){
			sum+=d_repeat[px->index-1]*px->value*px->value;
			++px;
		}			
...
Kernel::Kernel(int l, svm_node * const * x_, const svm_parameter& param)
  :l(l), num_kernels(param.num_kernels), max_index(param.features), fperk(param.fperk)
{
	kernels=new kernel[num_kernels];
	memcpy(kernels,param.kernels,sizeof(struct kernel)*num_kernels);
	
	srand ( time(NULL) );
	manage_master = new int[l];
	memset(manage_master, 0, sizeof(int)*l);
	d=new double[num_kernels];
	d_repeat = new double[max_index];
	for(int n=0;n<num_kernels;n++)
	{
		d[n]=(1.0/num_kernels);
		for(int i = 0; i<fperk; i++)
			if(n*fperk + i <max_index)
				d_repeat[n*fperk + i] = d[n];
	}
	data_feature = new double*[l];
	data_square  = new double*[l];
	for(int i=0;i<l;i++){
		data_feature[i] = new double[max_index];
		memset(data_feature[i], 0, sizeof(double)*max_index);
		data_square[i]	= new double[num_kernels];
		memset(data_square[i], 0, sizeof(double)*num_kernels);
	}
	...
	for(int i =0; i<l;i++){
		const svm_node *temp = x[i];
		if(temp->index == 0) ++temp;
		while(temp->index != -1){
			data_feature[i][temp->index-1] = temp->value; 
			++temp;
		}
	}
	int ker = 0;
	for(int i=0;i<l;i++){
		...
		for(int j=0;j<max_index;j++){
			sum+=pow(data_feature[i][j],2.0);
			if((j+1)%fperk == 0){
				data_square[i][ker] = sum;
				sum = 0.0;
				++ker;
			}
		}
		
			if(max_index%fperk != 0)
				data_square[i][ker] = sum;
			ker = 0;				
	}
}
...
Kernel::~Kernel()
{
  ...
  for(int n=0;n<num_kernels;n++)
    delete cache[n];   
  for(int n=0;n<l;n++){
    delete [] data_feature[n];
	delete [] data_square[n];
}   
delete [] data_feature;
delete [] data_square;
  delete[] cache;
  delete master_cache;
  delete [] manage_master;
}
			  ...
			  const svm_parameter& param)
{
	...
	while(px->index != -1 && py->index != -1)
	{
		if(px->index == py->index)
		{	
			if (param.d_repeat[px->index-1] > DTAU)
				sum+=param.d_repeat[px->index-1]*(px->value - py->value)*(px->value - py->value);
			++px;
			++py;
		}
		else
		{
			if(px->index > py->index){
				if (param.d_repeat[py->index-1] > DTAU)
					sum+=param.d_repeat[py->index-1]*py->value*py->value;
				++py;
			}
			else{
				if (param.d_repeat[px->index-1] > DTAU)
					sum+=param.d_repeat[px->index-1]*px->value*px->value;
				++px;
			}
		}			
	}
...
public:
  ...
  // Thanks to ashesh for a fix to this dual function
  double dual(){
    double r=-INF;
    for(int n=0;n<num_kernels;n++)
...
public:
  ...
  // end
	...
	void Solve(int l, float L_p, const QMatrix& Q, const double *p_, const schar *y_,
             ...
             int d_proj, double lambda_, double obj_threshold_, double diff_threshold_, const svm_parameter* param);
...
protected:
	...
	bool f1;
	bool f2;
	int *manage_master; // to manage master cache
	int monotone;
	int tune_precision;
	double obj_true;
  ...
  Timer fun_timer;       //store time spent in function evaluation 
    Timer grad_timer;       //store time spent in function evaluation 
  ...
  double *G;		 // gradient of objective function
	...
	Qfloat *QD;    // diagonal entries of the final kernel matrix 
  // end
  ...
  double Kdash(int feature);
    double Kdash1(int feature);

  void K_initialise(void);
    void d_gradient(void);
    void sleep(unsigned int msec);
    double value_idx(void);
...
private:
  ...
  // Generic spectral projected gradient solver 
...
//	  for(int i=0;i<l;i++)
 ...
 // delete [] sum_temp;
  ...
  double max_d=0;
  int number_of_kernels_selected=0;
  int number_of_Relevant_kernels=0;
  for(int n=0; n<num_kernels;n++){
	  if(d[n]>max_d)
		max_d=d[n];
	if(d[n]>0)
			number_of_kernels_selected++;
  }
  for(int n=0;n<num_kernels;n++){
	if(d[n]>0.001*max_d)
		number_of_Relevant_kernels++;
  }
  info("Number of Kernels Selected: %d \n",number_of_kernels_selected);
  info("Number of Relevant Kernels: %d \n",number_of_Relevant_kernels);
  
}

void Solver::QPSolver(SolutionInfo* si){  // original libsvm
...
while(1)
 ...
 /*void Solver::QPSolver(SolutionInfo* si)  // vishy QP
{
...
void Solver::LPQPSolver(SolutionInfo* si)
{
  ...
  g_from_Qalpha();
    ...
    double T2=0.0;
    for(int n=0;n<num_kernels;n++)
      T1+=pow(-grad_d[n]/lambda, (double)L_q);

    // if(fabs(T1) > TAU)
    // {
    //   T2=pow(T1, (2 - L_q)/L_q);
    //   T1=pow(T1, (2 - 2*L_q)/L_q);
    //   for(int n=0;n<num_kernels;n++)
    //   {
    //     if(-grad_d[n] < TAU)
    //       continue;
    //     double g_sq=dir_i*Qalpha_all[i][n] + dir_j*Qalpha_all[j][n];
    //     double mult=(2 - L_q)*T1*pow(-grad_d[n]/lambda, 2*L_q-2);
    //     mult+=(L_q - 1)*T2*pow(-grad_d[n]/lambda, L_q-2);
    ...
    if(fabs(T1) > TAU)
      {
        T1=(2-L_q)*pow(T1, -2.0/L_q)/lambda;
        for(int n=0;n<num_kernels;n++)
          {
            if(-grad_d[n] < TAU)
              continue;
            double g_sq=dir_i*Qalpha_all[i][n] + dir_j*Qalpha_all[j][n];
            T3 += g_sq*d[n];
            T2=-(L_q -1)*d[n]*g_sq*g_sq/grad_d[n];
            dir_hess+=T2; 
          }
        dir_hess+=T3*T3*T1;
      }
...
void Solver::EntSolver(SolutionInfo* si)
{
  ...
  info("initial obj=%f \n", -obj);
    ...
    Qfloat *rr=new Qfloat[num_kernels];
                      ...
                      SolutionInfo* si)
{
  ...
  info("\n \n");
  info("SVM calls: %d \n", fun_timer.num_calls);
  info("\nCPU time spent in SVM Call: %f \n", fun_timer.total_cpu);
    info("\nCPU time spent in Gradient: %f \n", grad_timer.total_cpu);
											 ...
											 proj_c *proj_fp)
{
  ...
  info("g_L2=%f pg_L2=%f Inf=%f ", grad_norm_l2, pg_norm_l2, pg_norm_inf);
  
 if(!((pg_norm_inf < ProjGrad::inf_abs_tol)||(pg_norm_l2 < ProjGrad::l2_abs_tol))){ 
    if(tune_precision == 1){
//if(pg_norm_l2 >=0.1 && f1){			 //
...
solver_timer.start();
  double my_alpha=1.0/norm_inf(gradk, num_kernels);
...
delete [] d_temp;
    ...
    while(true)
    {
      ...
      if(flag)
      {
        ...
        info("rho=%.4f ck=%.4f dtg=%4f ", rho,ck,dtg);
      }
      // Step 2.3 
	...
	if(use_SPG == 1){
		...
		printf("step=%.6f eps=%.6f ",step,eps);
        // Success in step 2.3
        // d_{k+1}=d_{+} 
        // s_{k}=d_{k+1} - d_{k}=step*dir_{k} 
        // y_{k}=g_{k+1} - g_{k} 
        
        double gradplus[num_kernels]; 
...
// info("Obj=%.4f \n", objplus);
      ...
      break;
        ...
        // double stepnew=-0.5*step*dtg/(objplus-obj-step*dtg);
        // if((stepnew < step*ProjGrad::sigma2) &&
        //    (stepnew >= ProjGrad::sigma1))
        //   step=stepnew;
        // else
        //   step *= ProjGrad::decay;
        continue;
    ...
    // Kill the experiment if its not over in 30min
   // solver_timer.stop();
   // if(solver_timer.total_cpu > 10000)
//		iter = ProjGrad::max_iter;
//	solver_timer.start();	
  }
  si->obj=-1;
  si->rho=0;
...
if(use_SPG == 1){
	...
	if(use_SPG == 1){
		...
		printf("step=%.6f eps=%.7f ",step,eps);
        // Success in step 2.3
        // d_{k+1}=d_{+} 
        // s_{k}=d_{k+1} - d_{k}=step*dir_{k} 
        // y_{k}=g_{k+1} - g_{k} 
...
void Solver::sleep(unsigned int msec){
	clock_t goal=msec+clock();
	while (goal<clock());
}
void Solver::swap_index(int i, int j)  // original libsvm
{
	Q->swap_index(i,j);
	swap(y[i],y[j]);
	swap(G[i],G[j]);
...
/*
double Solver::Kdash(int feature){
		int i,j;
		feature--;
		double total;
		total=0;
		const Qfloat *data1 = NULL;
		const Qfloat *data = NULL;
		double sum[l];
		...
		for(i=0;i<l;i++){
			...
			data=Q->get_Q(i,l);
			data1=Q->get_Qn(feature,i,l);
			for(j=0;j<l;j++){
				//sum[j]+=K_intermediate[i][j]*((double)data1[j])*alpha[i]*y[i];
				sum[j]+=((double)data[j])*y[j]*((double)data1[j])*alpha[i];	
			}
		}
		for(i=0;i<l;i++){
			total+=sum[i]*alpha[i]*y[i];
		}
		return (0.5*total);		
}

void Solver::K_initialise(void){
		int i,j;
		const Qfloat *data1 = NULL;
		for (i=0;i<l;i++){
			data1=Q->get_Q(i,l);
			for(j=0;j<l;j++){
				K_intermediate[i][j]=((double)data1[j])*y[i]*y[j];			
			}
		}
}

void Solver::d_gradient(void){	
	int n;
   for(n=0;n<(num_kernels);n++){
	grad_d[n]=Kdash(n+1);
	}
...
void Solver::d_gradient(void){			
		...
		for(i=0;i<l;i++){
			...
			for(int feature=0; feature<num_kernels;feature++){
				data1=Q->get_Qn(feature,i,l);
				for(j=0;j<l;j++){
					sum_temp[j][feature]+=((double)data[j])*y[j]*((double)data1[j])*alpha[i];	
				}
			}
		}
			for(j=0;j<num_kernels;j++){	
				total = 0.0;
				for(i=0;i<l;i++){
					total+=sum_temp[i][j]*alpha[i]*y[i];
				}
				grad_d[j] = 0.5*total;
			}	
}
...
void Solver::d_gradient(void){		
		...
		for(i=0;i<l;i++){
			Qfloat *data = NULL;
			data=Q->get_Q_K(i,l,alpha, inner);
			if(alpha[i] > 0){
			for(int feature=0; feature<num_kernels;feature++){
				data1=Q->get_Qn(feature,i,l);
		//		initial_timer1.start(); //timer
					for(j=0;j<l;j++){
						grad_d[feature]+=(double)(data[j]*data1[j]);	
					}
		//		initial_timer1.stop(); //timer
			}}
			delete [] data;
		}
...
void Solver::g_from_Qalpha(void){ // from original libsvm
	int i;
	for(i=0;i<l;i++)
	{
		G[i] = p[i];
...
double Solver::d_lp(void)
{
  ...
  for(int n=0;n<num_kernels;n++)
  {
    obj+=pow(-grad_d[n]/lambda,(double) L_q);
    // info("n=%d L_q=%f theta=%f obj in d_lp=%f \n", n, L_q, -grad_d[n]/lambda, obj);
  }
  double denom=pow(obj, (L_q - 2.0)/L_q);
  obj=pow(obj, 2.0/L_q);
  
  for(int n=0;n<num_kernels;n++)
    d[n]=pow(fabs(-grad_d[n]/lambda), L_q-1.0)/denom;
...
double Solver::primal_obj(bool reinit, obj_grad_c *obj_grad_fp){
  ...
  //K_initialise();  // Dont need it here
  g_from_Qalpha();
  SolutionInfo si;
  //printf("entering QPSolver ");
  QPSolver(&si);
  double obj= -si.obj;
  if(lambda)
...
int Solver::ent_select_working_set(int &out_i, int &out_j)
{
	...
	Qfloat *QQ_i=new Qfloat[active_size];
	Qfloat *QQD=new Qfloat[active_size];
	Qfloat *Qalpha=new Qfloat[active_size];
...
public:
	...
	{
		this->si=si;
		Solver::Solve(l,L_p,Q,p,y,alpha,Cp,Cn,eps,si,shrinking,solver_type, d_regularizer, d_proj, lambda_,obj_threshold_,diff_threshold_, param);
	}
private:
	SolutionInfo *si;
...
public:
	...
	{
		clone(y,y_,l);
		master_cache=new Cache(l,(long int)(param.master_cache_size*(1<<20)));
		...
		for(int i=0;i<l;i++)
    {
      QD[i]=0.0; // contains the diagnoal elements of final kernel matrix
     Qfloat k_value=(Qfloat) kernel_Prodfinal(i,i);  // returns (i,i) value of kernel 'n'
     QD[i]=(Qfloat)(k_value);
	...
	double kernel_ij(int i, int j) const { // return (i,j) of final kernel matrix
		return (double)(this->kernel_Prodfinal)(i,j);
	}
    ...
    {
      for(j=start;j<len;j++){
        //data[j]=(Qfloat)(this->kernel_all)(n*fperk+1,i,j);
	...
	Qfloat *get_Q(int i, int len) const
	{
		...
		int start=0;
		start = master_cache->get_data(i,&master_data,len);
		if(manage_master[i]==0){
			memset(master_data,0,sizeof(Qfloat)*len);
			manage_master[i] = 1;
			start = 0;
		}
		else
		...
		for(int j=start;j<len;j++){
			dd = (double)master_data[j];
			master_data[j] = (1.0/LUT[(int)(min(30.0,dd)*1000000)]);
			if(y[i] != y[j])
				master_data[j]*=-1;		
		}  
		}
	...
	Qfloat *get_Q_K(int i, int len, double *alpha1, bool &inner) const
	{
		Qfloat *master_data,*data;
		Qfloat *master_dummy;
		int start=0;
		master_dummy = new Qfloat[len];
		memset(master_dummy,0,sizeof(Qfloat)*len);
		if(alpha1[i] > 0){
			start = master_cache->get_data(i,&master_data,len);
		if(manage_master[i]==0){
			memset(master_data,0,sizeof(Qfloat)*len);
			...
			for(int j=0;j<len;j++){
				if(alpha1[j] > 0){
					if(j >= start){
						...
						master_dummy[j]=alpha1[i]*alpha1[j]*master_data[j];
				}
				else
					master_dummy[j] = 0.0;
			}  
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	s.Solve(l, param->L_p, *tmp, minus_ones, y,
          ...
          param->d_proj, param->lambda, param->obj_threshold, param->diff_threshold, param);
	double *d=tmp->get_d();
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	s.Solve(l, param->L_p, *tmp, zeros, ones,
          ...
          param->lambda, param->obj_threshold, param->diff_threshold, param);
...
{
		if(read_num_kernels==0){
				param->kernels=new kernel[param->num_kernels];
			for(int i=0;i<param->num_kernels;i++){
				param->kernels[i].gamma=1;
				param->kernels[i].kernel_type = 2;
				param->kernels[i].degree=0;
		...
		while(*s!='\0')
		{
			...
			switch(buffer2[1])
			{
				...
				case 'r':
					...
					break;
				case 'j':
					param->kernels[i].feature=((int)n + 1);	
					//printf("rk %d \n",param->kernels[i].feature);
					break;
        case 'w':
  ...
  if(param->d_proj == SIMPLEX)
	...
	for(int i=0;i<param->num_kernels;i++)
	{
		...
		if(param->kernels[i].kernel_type == PRECOMPUTED)
			...
			fprintf(fp,"-t %d -w %.17g -d 0 -j %d -g %.17g -r 0\n",param->kernels[i].kernel_type,
				param->kernels[i].coef,i,param->kernels[i].gamma);
	}
}
...
int svm_save_model(const char *model_file_name, const svm_model *model)
{
	...
	else
	{
		...
		fprintf(fp, "features_per_kernel %d\n", model->param->fperk);
		fprintf(fp, "num_features %d\n", model->param->features);
...
svm_model *svm_load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"features_per_kernel")==0){
			fscanf(fp,"%d",&model->param->fperk);
		}
		else if(strcmp(cmd,"num_features")==0){
			fscanf(fp,"%d",&model->param->features);
		}
	...
	// read sv_coef and SV
	int numker,fperk,max_index;
	max_index=model->param->features;
	fperk=model->param->fperk;
	model->param->d_repeat=new double[max_index];
	memset(model->param->d_repeat, 0, sizeof(double)*max_index);
	if(max_index % fperk == 0)
			numker=(int)(max_index/fperk);
	else
			numker=(int)(max_index/fperk) + 1;
	
	for(int n=0;n<model->param->num_kernels;n++)
	{
		int j = model->param->kernels[n].feature - 1;
		for(int i = 0; i<fperk; i++)
			if(j*fperk + i <max_index)
				model->param->d_repeat[j*fperk + i] = model->param->kernels[n].coef;;
	}
...
const char *svm_check_parameter(const svm_problem *prob, const svm_parameter *param)
{
  ...
  /*if(sum_d > 1.0+TAU || sum_d < 1.0-TAU)
  {
	...
	if(param->gmkl_type!=0 && param->gmkl_type!=1)
		return "Invalid formulation of kernels";
